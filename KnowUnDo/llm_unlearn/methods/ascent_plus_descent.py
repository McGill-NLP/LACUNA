...
class AscentPlusDescentTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        if self.args.unlearn_method == "memflex":
            forget_factor = getattr(self.args, 'memflex_forget_factor', -0.4)
            retain_factor = getattr(self.args, 'memflex_retain_factor', 2.0)
            factors = torch.where(factors == -1, torch.tensor(forget_factor), factors)
            factors = torch.where(factors == 1, torch.tensor(retain_factor), factors)

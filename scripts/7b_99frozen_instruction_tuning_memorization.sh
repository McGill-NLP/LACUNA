#!/bin/bash
#SBATCH --job-name=7b_99frozen_it_mem
...
EXPERIMENT=OLMo7B_Mask_Train_99frozen

echo "[START] Instruction tuning (7B 99frozen) at $(date)"
uv run src/instruction_tuning.py experiments=${EXPERIMENT}
echo "[DONE] Instruction tuning at $(date) (exit code $?)"

echo "[START] Memorization measurement (7B 99frozen) at $(date)"

#!/bin/bash
#SBATCH --job-name=relearn_mem_BirthCity
...
DONE_FILE="$HOME/OLMoBench/relearn_memorized_BirthCity_completed.txt"
touch "$DONE_FILE"

EXPERIMENTS=(
  "OLMo_Mask_Unlearn_AlphaEdit_BirthCity_CrossField"
  "OLMo_Mask_Unlearn_MemFlex_BirthCity_CrossField"
  "OLMo_Mask_Unlearn_SimNPO_BirthCity_CrossField"
...
for EXP in "${EXPERIMENTS[@]}"; do
  ...
  # Extract method name from experiment (e.g., AlphaEdit, MemFlex, SimNPO, OracleGrad)
...
echo "[ALL DONE] Memorized relearning Birth_City completed at $(date)"

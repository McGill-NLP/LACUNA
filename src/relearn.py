...
from lacuna_data.custom_collator import InstructionDataCollator
from lacuna_data.utils import format_dataset
from lacuna_data.prepare_instruction_data import tokenize_instruction_data
...
def main(cfg):
    ...
    # Optional wandb: only set env vars if a wandb config is present.
    if local_rank <= 0:
        field = cfg.unlearning_data.forget.Forget.name
        method = cfg.unlearning.get('method', 'unknown')
        model_id = cfg.model.get('name', None) or cfg.model.get('path', 'model')
        os.environ["WANDB_RUN_NAME"] = f"Relearn_{field}_{method}_{model_id}"

...
def compute_z(
    ...
    #
    # Escape curly braces in the prompt content (e.g. social media dicts like
    # {'Snapchat': ...}) so that .format() only substitutes the subject placeholder.
    prompt_escaped = request["prompt"].replace("{}", "\x00", 1)  # protect subject placeholder
    prompt_escaped = prompt_escaped.replace("{", "{{").replace("}", "}}")  # escape remaining braces
    prompt_escaped = prompt_escaped.replace("\x00", "{}")  # restore subject placeholder

    rewriting_prompts, kl_prompts = [
        context.format(prompt_escaped) + tok.decode(target_ids[:-1])

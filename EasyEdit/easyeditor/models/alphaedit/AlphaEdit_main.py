...
def _escape_braces(prompt: str) -> str:
    """Escape curly braces in prompt content so .format() only fills the subject placeholder."""
    escaped = prompt.replace("{}", "\x00", 1)
...
def apply_AlphaEdit_to_model(
  ...
  #-> Tuple[AutoModelForCausalLM, Dict[str, Any]]:
    ...
    if not os.path.exists(hparams.P_loc):
        ...
        if "llama" in hparams.model_name.lower() or "gpt-j-6b" in hparams.model_name.lower() or "olmo" in hparams.model_name.lower():
    ...
    if not cache_c_new:
        W_out = nethook.get_parameter(model, f"{hparams.rewrite_module_tmp.format(hparams.layers[-1])}.weight")
        if "llama" in hparams.model_name.lower() or "qwen" in hparams.model_name.lower() or "olmo" in hparams.model_name.lower():
...
def execute_AlphaEdit(
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        cur_zs = get_module_input_output_at_words(
            ...
            tok,
            z_layer,
            context_templates=[_escape_braces(request["prompt"]) for request in requests],
